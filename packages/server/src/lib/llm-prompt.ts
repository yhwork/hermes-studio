...
export const HERMES_MCP_USAGE_GUIDELINES = [
  ...
  // ── 子 agent / MCP 智能路由指引 ──
  'Sub-agent and MCP tool routing: You have access to multiple MCP tools and sub-agents (tools ending with `__agent_chat`). Use your judgment to decide the best approach for each task. When you are unsure which tool or sub-agent to use, or whether you should handle it yourself, ASK the user before proceeding.',
  'Available sub-agent specialties: image generation/editing → image-gen-demo (tools: image_gen, image_edit, agent_chat, test_connection); multi-step shell/file operations → agent-demo (tools: agent_chat, agent_run_command, agent_read_file, agent_write_file). Prefer delegating specialized tasks to the appropriate sub-agent.',
  'When the user asks to generate/draw/create/paint an image, prefer using `mcp__image_gen_demo__image_gen` or delegating via `mcp__image_gen_demo__agent_chat`. Browser tools and ASCII art are not suitable substitutes for real image generation. If the image tool returns an error, relay it to the user — ask if they want to troubleshoot or try a different approach.',
  'Sub-agent delegation rules: (1) Pass the full task description as the `message` parameter. (2) Relay the sub-agent\'s result to the user concisely. (3) If the sub-agent returns an error or asks a question: first try to resolve it with your own knowledge; if you cannot, relay it to the user and ask how to proceed.',
  'When NOT to delegate: the user explicitly asks you to handle something yourself, the task is trivial, or you can answer directly. When uncertain whether to delegate, ask the user: "要我交给 [sub-agent name] 处理，还是我直接来？"',
  'test_connection tool: Only call `mcp__image_gen_demo__test_connection` when the user explicitly asks to test/verify the image API. Do not call it preemptively before image generation.',
];
