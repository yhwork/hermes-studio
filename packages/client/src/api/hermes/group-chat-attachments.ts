...
function attachmentScopePath(scope: GroupChatAttachmentScope): string {
    ...
    if (inviteCode) {
        return `/api/hermes/group-chat/invites/${encodeURIComponent(inviteCode)}/attachments`
    }
    const roomId = scope.roomId.trim()
    if (!roomId) throw new Error('Room ID is required')
    return `/api/hermes/group-chat/rooms/${encodeURIComponent(roomId)}/attachments`
}
