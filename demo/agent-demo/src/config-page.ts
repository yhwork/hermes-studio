 ...
 * 页面逻辑：
 *   1. GET  /api/tool-descriptions       拉取 { tools: [{name, defaultDescription, description, overridden}] }
 *   2. PUT  /api/tool-descriptions       提交 { name: description } 整表覆盖
 *   3. 保存后到 Hermes Studio MCP 面板点 agent-demo 的「重载」即生效
 */
...
<style>
  ...
  .reset { font-size: 11px; color: #2563eb; background: none; border: none; padding: 0; cursor: pointer; text-decoration: underline; }
...
<body>
  <h1>agent-demo · MCP 工具描述配置</h1>
  <div class="hint">编辑后点「保存全部」，再到 Hermes Studio 的 MCP 面板对 <code>agent-demo</code> 点「重载」即可生效。<br/>覆盖项写入：<span id="file">${file}</span></div>
...
function render() {
  ...
  root.querySelectorAll('.reset').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      const def = state.tools.find(t => t.name === name).defaultDescription;
      ta.value = def;
...
async function save() {
  ...
  if (r.ok) {
    state.dirty = false;
    document.getElementById('status').textContent = '已保存 ✓  去 Hermes Studio MCP 面板重载 agent-demo';
    await load();
  } else {
...
function resetAll() {
  ...
  document.querySelectorAll('textarea[data-name]').forEach(ta => {
    const def = state.tools.find(t => t.name === ta.dataset.name).defaultDescription;
    ta.value = def;
  });
  state.dirty = true;
  document.getElementById('save').disabled = false;
...
async function saveModel() {
  ...
  document.getElementById('modelStatus').textContent = r.ok ? '已保存 ✓  到 Hermes MCP 面板重载 agent-demo 生效' : '保存失败';
  await loadModel();
}
