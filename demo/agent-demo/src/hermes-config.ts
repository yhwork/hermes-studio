 ...
 *
 * 解析的字段：
 *   model.default              → 默认模型 id
 *   model.provider             → 供应商名（去 "custom:" 前缀）
 *   providers.<name>.api       → base URL
 *   providers.<name>.api_key   → API key
 *   providers.<name>.transport → 协议（anthropic_messages / chat_completions / ...）
 *   providers.<name>.default_model → 模型名（优先于 model.default）
 */
...
export interface MainAgentModelConfig {
  ...
  model: string;
  provider: string;
  apiMode?: string; // transport，交给 ekko 的 requestStyleForConfig 映射成 requestStyle
  profile: string;
  configPath: string;
}
...
/** 读取主 agent 模型配置；读不到 / 解析失败返回 null。 */
export function loadMainAgentModelConfig(): MainAgentModelConfig | null {
  ...
  try {
    ...
    // api_key 内联，或 api_key_env 引用环境变量
