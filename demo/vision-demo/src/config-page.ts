...
/**
 * /config 页面 —— 在线编辑各 MCP 工具的描述 + 子 agent 模型配置。
 * 保存后到 Hermes Studio MCP 面板点 vision-demo 的「重载」即生效。
 */
...
<body>
  <h1>vision-demo · MCP 工具描述配置</h1>
  <div class="hint">编辑后点「保存全部」，再到 Hermes Studio 的 MCP 面板对 <code>vision-demo</code> 点「重载」即可生效。<br/>覆盖项写入：<span id="file">${file}</span></div>
...
async function save() {
  ...
  if (r.ok) {
    state.dirty = false;
    document.getElementById('status').textContent = '已保存 ✓  去 Hermes Studio MCP 面板重载 vision-demo';
    await load();
  } else {
...
async function saveModel() {
  ...
  document.getElementById('modelStatus').textContent = r.ok ? '已保存 ✓  到 Hermes MCP 面板重载 vision-demo 生效' : '保存失败';
  await loadModel();
}
